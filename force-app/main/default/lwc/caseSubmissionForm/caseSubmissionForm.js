...
export default class CaseSubmissionForm extends NavigationMixin(
  LightningElement
) {
  ...
  get isValid() {
    return (
      this.caseModel.subject &&
      this.caseModel.description &&
      this.caseModel.priority
    );
  }
  ...
  handleRenewContract() {
    ...
    let url =
      "https://ccpsupport-dev-ed.my.site.com/renewcontract/s/?" +
      "orgId=" +
      this.orgInfo.orgId.substring(0, 15) +
      "&firstName=" +
      this.orgInfo.userFirstName +
      "&lastName=" +
      this.orgInfo.userLastName +
      "&email=" +
      this.orgInfo.userEmail;
    console.log("url: " + url);
    ...
    if (this.isValid) {
      ...
      this.dispatchEvent(
        new ShowToastEvent({
          title: "Error",
          message:
            "Subject, Description and Priority are required for submission.",
          variant: "error",
        })
      );
    }
