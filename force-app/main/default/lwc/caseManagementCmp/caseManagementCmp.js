...
// Desc:
// - COLUMNS modification - update case estimation status and case complete button 
...
const COLUMNS = [
  {
    ...
    type: "customCaseNumber",
    sortable: true,
    hideDefaultActions: true,
    wrapText: true,
    typeAttributes: { caseId: { fieldName: "id" } },
  },
  {
    label: "Subject",
    fieldName: "subject",
  ...
  },
  {
    ...
    type: "customCaseEstimationStatus",
    sortable: true,
    cellAttributes: { alignment: "left" },
    typeAttributes: {
      caseId: {
      ...
      caseEstimationStatus: {
        fieldName: "estimationStatus",
      },
      caseEstimationHours: {
        fieldName: "estimationHours",
      }
...
export default class CaseManagementCmp extends LightningElement {
  ...
  handleCaseEstimationStatusSelect(event) {
    const id = event.detail.caseId;

    switch (event.detail.caseEstimationStatus) {
      case "approved":
        this.handleApproveEstimate(id);
        break;
      case "rejected":
        this.handleRejectEstimate(id);
        break;
    }
  }
  ...
  handleOnCaseNumberClick(event) {
    console.log('handleOnCaseNumberClick');
